...
int main(int argc, char *argv[]) {
  ...
  check(argc == 3,
        ...
        "run this after to add the mask out the identified cloudy and snowy "
        "pixels\n"
        "   gdal_calc.py -A mask.tif -B ~/landsat.tif --calc=\"A*B\" "
        "--allBands=B --overwrite --type=Byte");
  ...
  out = GDALCreate(GDALGetDriverByName("GTiff"), argv[2], width, height, 1,
                   GDT_UInt16, NULL);
  ...
  for (int y = 0; y < height; y++) {
    printf("row: %i\r", y + 1);
    fflush(stdout);
    GDALRasterIO(GDALGetRasterBand(mask, 1), GF_Read, 0, y, width, 1, scanline,
                 width, 1, GDT_UInt16, 0, 0);
    ...
    GDALRasterIO(GDALGetRasterBand(out, 1), GF_Write, 0, y, width, 1,
                 tiny_scanline, width, 1, GDT_Byte, 0, 0);
  }
  puts("");
